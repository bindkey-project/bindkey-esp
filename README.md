...
1. **Le proxy matériel BindKey** — le boîtier physique qui assure l'authentification biométrique locale, la dérivation des clés via un secure element ATECC608A, et le chiffrement à la volée des données. Composé de deux microcontrôleurs ESP32-S3 :
   - un **master** (émulation USB MSC + biométrie + crypto AES-GCM + élément sécurisé) — dépôt [`bindkey-tinyesp`](https://github.com/bindkey-project/bindkey-tinyesp)
   - un **slave** (pilotage du vrai média physique en USB Host) — dépôt [`bindkey-esp`](https://github.com/bindkey-project/bindkey-esp) **← ce dépôt**
...
Ce dépôt contient le firmware **slave** (ESP32-S3 N°2) — l'une des deux MCU qui composent le proxy matériel BindKey.
